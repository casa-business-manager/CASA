...
public class SignInAndUpController {
    ...
    public ResponseResult<Object> handleSignUp(@RequestParam(value = "username") String username,
        ...
        if (StringUtils.isAnyBlank(username, password)) {
            ...
            responseResult.setMessage("Please Check Inputs Again!");
        ...
        if (username.length() < 6) {
            responseResult.setStatus(GlobalConstant.ERROR);
            responseResult.setMessage("Username must be longer than 6 character!");
            return responseResult;
        }

        if (password.length() < 6) {
            responseResult.setStatus(GlobalConstant.ERROR);
            responseResult.setMessage("Password must be longer than 6 character!");
            return responseResult;
        }

        if (CheckPasswordStrength.check(password) < 4) {
            responseResult.setStatus(GlobalConstant.ERROR);
            responseResult.setMessage("Password Strength is Weak! Please use symbols, digits and letters!");
            return responseResult;
        }
        ...
        try {
            ...
            responseResult.setStatus(GlobalConstant.ERROR);
            responseResult.setMessage("Error, Please Retry!");
        }
        return responseResult;
    }
}
